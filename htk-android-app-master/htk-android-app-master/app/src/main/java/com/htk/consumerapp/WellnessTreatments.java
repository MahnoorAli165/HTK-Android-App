...
public class WellnessTreatments extends AppCompatActivity implements View.OnClickListener, OnMapReadyCallback, LocationListener, GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks, GoogleMap.OnMarkerDragListener {

    private EditText appointmentDateText, appointmentTimeText, specialNoteText, searchText;
    ;
    private String appointment_date, appointment_time, treatment_type, specialnote, customer_id, appointment_id = "";
    private Spinner treatmentTypeSpinner;
    private CheckBox receiveEmail;
    private Button addRecord, locationButton, confirmLocationButton;
    private boolean checked = false;
    private final String TAG = "Wellness Treatment";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wellness_treatments);
        ...
        mapFragment = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.mapW);
    ...
    public void initializViews(){
        appointmentDateText = findViewById(R.id.appointmentDateW);
        appointmentTimeText = findViewById(R.id.appointmentTimeW);
        treatmentTypeSpinner = findViewById(R.id.treatmentTypeSpinner);
        specialNoteText = findViewById(R.id.editText13);
        receiveEmail = findViewById(R.id.wellnessCheckBox);
        addRecord = findViewById(R.id.wellnessAddButton);
        progressBar = findViewById(R.id.progressBar8);

        progressBarUpload = findViewById(R.id.progressbarUploadW);
        attachButton = findViewById(R.id.attachBW);
        filenameLabel = findViewById(R.id.filenameW);

        layout = findViewById(R.id.wellnessLayout);
        locationLabel = findViewById(R.id.locationLabelW);
        locationButton = findViewById(R.id.locationButtonW);
        confirmLocationButton = findViewById(R.id.confirmLocationButtonW);
        searchText = findViewById(R.id.searchTextW);
        searchIcon = findViewById(R.id.searchIconW);
        center_pin = findViewById(R.id.center_pinW);
    }
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == appointmentDateText.getId()){
            if (isMapDisplayed) {
                ...
                new DatePickerDialog(WellnessTreatments.this, appointment_date_listener, myCalendar
            ...
            if (isMapDisplayed) {
                ...
                mTimePicker = new TimePickerDialog(WellnessTreatments.this, new TimePickerDialog.OnTimeSetListener() {
                    @Override
        ...
        else if(view.getId() == addRecord.getId()){
            ...
            if (isConnectedToInternet()) {
                if (isMapDisplayed) {
                    ...
                    Log.e(TAG, "Spinner says: " + treatmentTypeSpinner.getSelectedItem().toString());
                    if (treatmentTypeSpinner.getSelectedItem().toString().equalsIgnoreCase("Select Treatment") && appointmentDateText.getText().toString().equals("") && appointmentTimeText.getText().toString().equals("")) {
                        Toast.makeText(this, "You need to enter details to submit request", Toast.LENGTH_SHORT).show();
                    } else {
                        if (treatmentTypeSpinner.getSelectedItem().toString().equalsIgnoreCase("Select Treatment")) {
                            Toast.makeText(this, "Please select Treatment type", Toast.LENGTH_SHORT).show();
...
//                            mGoogleMap.addMarker(userMarkerOptions);
    ...
    public void bookAppointment(){
        ...
        treatment_type = treatmentTypeSpinner.getSelectedItem().toString();

        if (dataIntent != null) {
            uploadFile(dataIntent.getData());
        }

        final DatabaseReference reference = FirebaseDatabase.getInstance().getReference().child("appointments").child("wellness_treatments").push();
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                reference.child("treatment_type").setValue(treatment_type).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        hideProgressBar();
                        Toast.makeText(WellnessTreatments.this, "Request submitted", Toast.LENGTH_SHORT).show();
                        Log.e(TAG, line+"Request submitted\nID: "+appointment_id+"\nDate: "+appointment_date+"\nTime: "+appointment_time+line);
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(WellnessTreatments.this, "Error.\n"+databaseError, Toast.LENGTH_SHORT).show();
            }
        });
    }

    public void getAppointmentCount(){
        final DatabaseReference countReference = FirebaseDatabase.getInstance().getReference().child("appointments").child("wellness_treatments");
        countReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                childrenCount = (int) dataSnapshot.getChildrenCount();
                Log.e(TAG, "There are " + childrenCount + " Wellness Treatment requests already");
                if (childrenCount < 10) {
                    childrenCount += 1;
                    appointment_id = "htk-wel-00" + childrenCount;
                } else if (childrenCount >= 10 && childrenCount < 100) {
                    childrenCount += 1;
                    appointment_id = "htk-wel-0" + childrenCount;
                }
                else{
                    childrenCount += 1;
                    appointment_id = "htk-wel-" + childrenCount;
                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_PDF_CODE && resultCode == RESULT_OK && data != null && data.getData() != null) {
            ...
            if (data.getData() != null) {
                ...
                findViewById(R.id.attachment_iconW).setVisibility(View.VISIBLE);
...
//        startActivity(new Intent(WellnessTreatments.this, HomeActivity.class));
//    }
}
